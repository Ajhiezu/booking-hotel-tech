...
@section('page-title', 'Global Bookings')
...
<div class="bg-white rounded-2xl border border-gray-100 p-5 mb-6">
    <form action="{{ route('admin.bookings.index') }}" method="GET" class="flex flex-wrap gap-3 items-end">
        <div>
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Booking Code..." class="border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-48">
        </div>
        <div>
        ...
        <a href="{{ route('admin.bookings.index') }}" class="text-sm text-gray-500 py-2">Clear</a>
    </form>
</div>
            ...
            <tbody class="divide-y divide-gray-50">
                @forelse($bookings as $booking)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4">
                            <a href="{{ route('admin.hotels.show', $booking->hotel_id) }}" class="text-blue-600 hover:underline">{{ $booking->hotel?->name }}</a>
                        </td>
                        <td class="px-6 py-4 text-gray-500 text-xs text-center whitespace-nowrap">
